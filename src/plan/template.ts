import type { WorkoutDayInput, Intensity, WorkoutTag, ActivityType } from '../types';
import { addDays, format, getDay } from 'date-fns';

// Plan start date: January 2, 2026
const PLAN_START = new Date(2026, 0, 2); // January 2, 2026
const PLAN_END = new Date(2026, 4, 2); // May 2, 2026
...
// Helper for Wednesday Squash sessions
const squash = (dayOffset: number, week: number, phase: string): WorkoutDayInput => ({
  date: getDate(dayOffset),
  title: 'Squash - 1 Hour',
  details: `1-Hour Squash Workout Summary

A full 60-minute squash session consisting entirely of match play, without a formal warm-up. Played 5–8 competitive matches against a friend, keeping a steady pace with short breaks only between games. The intensity stayed consistently high, focusing on endurance, quick direction changes, sprint bursts, court positioning, and shot selection under pressure. The repeated games helped build stamina, improve reaction speed, and sharpen strategic thinking while maintaining a competitive rhythm throughout the hour.

We end the session with a Duvel in the bar. 🍺`,
  phase,
  week,
  intensity: 'S',
  tags: ['steady'],
  ...
  planned_distance_km: null,
  activity_type: 'squash',
});

// Calculate all Wednesdays in the plan period
const getWednesdayOffsets = (): number[] => {
  const wednesdays: number[] = [];
  let current = new Date(PLAN_START);
  let dayOffset = 0;
  
  while (current <= PLAN_END) {
    // getDay() returns 0 for Sunday, 1 for Monday, ..., 3 for Wednesday
    if (getDay(current) === 3) { // Wednesday
      wednesdays.push(dayOffset);
    }
...
// Get week number for a day offset
const getWeekNumber = (dayOffset: number): number => {
  return Math.floor(dayOffset / 7) + 1;
};

// Get phase for a week number
const getPhaseForWeek = (week: number): string => {
  if (week <= 4) return 'Phase 1';
  if (week <= 8) return 'Phase 2';
  if (week <= 13) return 'Phase 3';
  if (week <= 16) return 'Phase 4';
  return 'Phase 5';
};
...
export const generatePlanTemplate = (): WorkoutDayInput[] => {
  ...
  // Get all Wednesday offsets for Squash sessions
  const wednesdayOffsets = getWednesdayOffsets();
  ...
  // Day 5 is Wednesday - will be replaced with Squash below
  plan.push(workout(6, 'Long Run (Easy)',
    ...
    'Phase 1', 1, 'E', ['easy', 'longrun'], 35, null, 'run'));
  ...
  // Day 33 is Wednesday - will be replaced with Squash below
  plan.push(workout(34, 'Long Run (Easy)',
    '50 min → 4/1 ratio\n\nWarm-up: 5-8 min brisk walk + very easy jog\nMain: 4 min run / 1 min walk\nCool-down: 5 min walk\n\nBiggest long run yet! Stay patient and conversational.',
    'Phase 2', 5, 'E', ['easy', 'longrun'], 50, null, 'run'));
  ...
  // Day 40 is Wednesday - will be replaced with Squash below
  plan.push(workout(41, 'Long Run (Easy)',
    '55 min → 5/1 ratio\n\nWarm-up: 5-8 min brisk walk + very easy jog\nMain: 5 min run / 1 min walk\nCool-down: 5 min walk\n\nGreat endurance building here!',
    'Phase 2', 6, 'E', ['easy', 'longrun'], 55, null, 'run'));
  ...
  plan.push(workout(59, 'Run B (Steady Blocks)',
    ...
    'Phase 3', 9, 'S', ['steady'], 33, null, 'run'));
  plan.push(rest(60, 9, 'Phase 3'));
  // Day 61 is Wednesday - will be replaced with Squash below
  plan.push(workout(62, 'Long Run (Easy)',
    ...
    'Phase 3', 9, 'E', ['easy', 'longrun'], null, 8, 'run'));
  ...
  plan.push(workout(78, 'Run A (Easy) - Deload',
    ...
    'Phase 3', 12, 'E', ['easy', 'deload'], 37, null, 'run'));
  ...
  plan.push(workout(80, 'Run B (Easy + Strides) - Deload',
    ...
    'Phase 3', 12, 'E', ['easy', 'deload'], 32, null, 'run'));
  plan.push(rest(81, 12, 'Phase 3'));
  // Day 82 is Wednesday - will be replaced with Squash below
  plan.push(workout(83, 'Long Run (Easy) - Deload',
    '8-9 km easy\n\nWarm-up: 5-8 min brisk walk + very easy jog\nMain: 8-9 km easy\nCool-down: 5 min walk',
    'Phase 3', 12, 'E', ['easy', 'longrun', 'deload'], null, 8.5, 'run'));
  
  // Week 13 (Day 84-90, Mar 27 - Apr 2)
  plan.push(rest(84, 13, 'Phase 3'));
  plan.push(workout(85, 'Run A (Easy)',
    '45 min easy\n\nWarm-up: 5-8 min brisk walk + very easy jog\nMain: Easy running\nCool-down: 5 min walk',
    'Phase 3', 13, 'E', ['easy'], 45, null, 'run'));
  ...
  plan.push(workout(101, 'Run B (Tempo)',
    ...
    'Phase 4', 15, 'T', ['tempo'], 40, null, 'run'));
  plan.push(rest(102, 15, 'Phase 4'));
  // Day 103 is Wednesday - will be replaced with Squash below
  plan.push(workout(104, 'Long Run (Easy)',
    ...
    'Phase 4', 15, 'E', ['easy', 'longrun'], null, 14.5, 'run'));
  ...
  plan.push(workout(108, 'Run B (Steady Blocks)',
    ...
    'Phase 4', 16, 'S', ['steady'], 39, null, 'run'));
  plan.push(rest(109, 16, 'Phase 4'));
  // Day 110 is Wednesday - will be replaced with Squash below
  plan.push(workout(111, 'Long Run (Easy)',
    ...
    'Phase 4', 16, 'E', ['easy', 'longrun'], null, 16, 'run'));
  ...
  plan.push(workout(113, 'Run A (Easy) - Taper',
    ...
    'Phase 5', 17, 'E', ['easy', 'deload'], 37, null, 'run'));
  ...
  plan.push(workout(115, 'Run B (Easy + Strides) - Taper',
    ...
    'Phase 5', 17, 'E', ['easy', 'deload'], 27, null, 'run'));
  plan.push(rest(116, 17, 'Phase 5'));
  // Day 117 is Wednesday - will be replaced with Squash below (but it's close to race day)
  ...
  plan.push(workout(119, 'Easy Shakeout',
    ...
    'Phase 5', 17, 'E', ['easy', 'deload'], 22, null, 'run'));
  plan.push(workout(120, '🏃 RACE DAY - Bosbeekse 15',
    ...
    'Phase 5', 17, 'S', ['race'], null, 15, 'run'));
  
  // Now add Squash sessions for all Wednesdays
  for (const dayOffset of wednesdayOffsets) {
    const week = getWeekNumber(dayOffset);
    const phase = getPhaseForWeek(week);
    
    // Don't add squash on the last Wednesday if it's too close to race day
    if (dayOffset <= 115) { // Skip if within 5 days of race
      plan.push(squash(dayOffset, week, phase));
    }
  }
